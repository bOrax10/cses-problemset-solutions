...
void solve()
{
    ...
    for (auto it : edges)
    {
        ...
        if (dist[u] != LLONG_MAX && dist[u] + wt < dist[v])
        {
            negativeCycleVertex = u;
            break;
        }
    }
    // print(negativeCycleVertex);
    ...
    else
    {
        ...
        for (int i = 0; i < V; ++i)
        {
            negativeCycleVertex = predecessor[negativeCycleVertex];
        }

        vector<int> cycle;
        int vertex = negativeCycleVertex;
        do
        {
            cycle.push_back(vertex);
            vertex = predecessor[vertex];
        } while (vertex != negativeCycleVertex);
        cycle.push_back(negativeCycleVertex);
