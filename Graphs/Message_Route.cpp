...
void bfs(int n, int source, vector<int> &parent, vector<int> adj[])
{
    queue<int> q;
    parent[source] = -1;
    q.push(source);
...
void backtrack(int node, vector<int> &parent, vector<int> &ans)
{
    if (parent[node] == -1)
    {
        ans.push_back(node);
    ...
    ans.push_back(node);
    backtrack(parent[node], parent, ans);
}
