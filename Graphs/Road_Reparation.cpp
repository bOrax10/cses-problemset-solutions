...
ll spanningTree(ll V, vector<vector<ll>> adj[])
{
    ...
    vector<ll> vis(V, 0);
    // {wt, node} because in the problem we only have to find the sum of the weights
    // if you want to find the MST, then keep it as {wt,node,parent} and push {0,0,-1};
    pq.push({0, 1});
    ll sum = 0;
    while (!pq.empty())
    {
    ...
    // if any node is not visited then it is not connected to other nodes so MST can't be formed.
    bool flag = 0;
    rep(i, 1, V)
    {
        if (vis[i] == 0)
